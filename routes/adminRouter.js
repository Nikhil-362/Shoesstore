...
router.get("/", isOwner, async (req, res) => {
    try {
        ...
        res.render("createProd", { products: admin.products,user: req.session.findUser });
    } catch (error) {
...
router.post('/delete/:id', async (req, res) => {
    try {
        ...
        const admin = await findOneAndDelete({ email: req.userOwner.email }, { products: req.params.id })
        console.log(admin);
